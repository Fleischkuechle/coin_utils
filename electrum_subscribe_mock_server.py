...
block3 = {
    "height": 2350327,
    "hex": "0000552920c2d8311e12d3e35b8da48ad29b1254e0a0d2be1623d717f69000000000000001aaf14e207eea7f36cdc1cf92a8d43a5db2ac1ce22925e104ccedca3b5d2d26892544630194331933c10227",
}
...
def reset_cycles() -> None:
    cycles["scripthash"] = itertools.cycle(
        [first_conn_scripthash, second_conn_scripthash]
    )
    ...
    while not writer.transport.is_closing():
        ...
        if msg:
            ...
            if method == "server.version":
                ...
                response = {"result": scripthash_status[0], "id": msg_id}
                await send_message(writer, response)
                await asyncio.sleep(2)
                response = {"method": method, "params": params + [scripthash_status[1]]}
                await send_message(writer, response)
            elif method == "blockchain.headers.subscribe":
                blocks = next(cycles["block"])
                response = {"result": blocks[0], "id": msg_id}
                await send_message(writer, response)
                await asyncio.sleep(2)
                response = {"method": method, "params": [blocks[1]]}
                await send_message(writer, response)
            else:
                response = {
                    "error": {
                        "code": -32601,
                        "message": f"Method {method} is not supported by this mock server",
                        "id": msg_id,
                    }
                }
                await send_message(writer, response)
...
async def get_server(
    fut: Union[asyncio.Future, Future], host: str = "127.0.0.1", ssl: bool = False
):
    port = 44445 if ssl else 44444
    context = create_default_context(purpose=Purpose.SERVER_AUTH) if ssl else None
    server = await asyncio.start_server(handle_rpc, host, port, ssl=context)
...
async def run_server_until_cancelled(
    fut: Future, stop_fut: Future, host: str = "127.0.0.1", ssl: bool = False
):
    stop_fut_async = asyncio.wrap_future(stop_fut)
    server = await get_server(fut, host=host, ssl=ssl)
    async with server:
        await asyncio.wait(
            [asyncio.create_task(server.serve_forever()), stop_fut_async],
