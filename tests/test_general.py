...
# the imports only work if the file is at the right place
import cryptos.ripemd as ripemd

# from cryptos import ripemd as ripemd
from cryptos import *
...
class TestECCArithmetic(unittest.TestCase):
    ...
    def test_all(self):
        for i in range(8):
            print("### Round %d" % (i + 1))
            x, y = random.randrange(start=2**256), random.randrange(start=2**256)
            self.assertEqual(
...
class TestElectrumWalletInternalConsistency(unittest.TestCase):
    ...
    def test_words_to_seed_password(self):
        seed = mnemonic_to_seed(self.words, passphrase_bytes=self.passphrase)
    ...
    def test_seed_to_keys(self):
        ...
        self.assertEqual(
            coin.privtoaddr(priv), coin.electrum_address(self.seed, 345, 1)
        )
    ...
    def test_master_public_key(self):
        ...
        self.assertEqual(
            coin.electrum_address(mpk, 0),
            coin.electrum_address(self.seed_from_password, 0),
        )
        ...
        self.assertEqual(
            coin.electrum_address(mpk, 101),
            coin.electrum_address(self.seed_from_password, 101),
        )
        ...
        self.assertEqual(
            coin.electrum_address(mpk, 0, 1),
            coin.electrum_address(self.seed_from_password, 0, 1),
        )
        ...
        self.assertEqual(
            coin.electrum_address(mpk, 0, 200),
            coin.electrum_address(self.seed_from_password, 0, 200),
        )
        self.assertEqual(
            Dash().electrum_address(mpk, 0, 200),
            Dash().electrum_address(self.seed_from_password, 0, 200),
        )
...
class TestTransactionSignVerify(unittest.TestCase):
    ...
    def test_all(self):
        ...
        for i in range(10):
            ...
            self.assertTrue(ecdsa_tx_verify(msg, sig, pub), "Verification error")

            self.assertIn(pub, ecdsa_tx_recover(msg, sig), "Recovery failed")
...
class TestTransaction(unittest.TestCase):
    ...
    def test_all(self):
        ...
        self.assertTrue(
            verify_tx_input(tx1, 0, mscript, sig1, pubs[1]), "Verification Error"
        )

        sig3 = multisign(tx, 0, mscript, privs[3])
        self.assertTrue(
            verify_tx_input(tx1, 0, mscript, sig3, pubs[3]), "Verification Error"
        )
    ...
    def test_multisig(self):
        ...
        self.assertEqual(
            c.p2sh_scriptaddr(script), "33byJBaS5N45RHFcatTSt9ZjiGb6nK4iV3"
        )

        self.assertEqual(
            t.p2sh_scriptaddr(script), "2MuABMvWTgpZRd4tAG25KW6YzvcoGVZDZYP"
        )
...
class TestBIP0032(unittest.TestCase):
    ...
    def _full_derive(self, key, chain, prefixes=(MAINNET_PRIVATE, MAINNET_PUBLIC)):
        if len(chain) == 0:
            ...
            return self._full_derive(
                bip32_privtopub(key, prefixes), chain[1:], prefixes
            )
        else:
            return self._full_derive(
                bip32_ckd(key, [chain[0]], prefixes), chain[1:], prefixes
            )
    ...
    def test_all_testnet(self):
        ...
        for tv in test_vectors:
            left, right = (
                self._full_derive(mk, tv[0], (TESTNET_PRIVATE, TESTNET_PUBLIC)),
                tv[1],
            )
            self.assertEqual(
                left,
