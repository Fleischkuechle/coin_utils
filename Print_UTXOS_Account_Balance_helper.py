...
class Print_UTXOS_Account_Balance_Merkle_Proof_helper:
    ...
    def balance_online_lookup(
        ...
        try:
            # coin.get_balance
            electrumXTx_list: ElectrumXUnspentResponse = coin.unspent(addr=pub_address)
            if print_result:
    ...
    def format_coin_value(self, value: float) -> str:
    ...
    def pretty_print_tx(
        ...
        print(" ")
        print(self.line_symbol * self.line_length)
        print(f"ElectrumXTx list. {coin_symbol} Unspent Transaction Outputs (UTXOs)")
        print(self.line_symbol * self.line_length)
        ...
        for electrumXTx in electrumXTx_list:
            ...
            coin_value: float = electrumXTx["value"] / atomic_to_coin_divider

            coin_value_formated: str = self.format_coin_value(value=coin_value)

            print(
                f"{'Value:':<{other_padding}}{electrumXTx['value']:<{info_padding}}{atomic} ({coin_symbol} {coin_value_formated}) ({coin_symbol}=atomic/100 000 000)"
            )
        ...
        account_ballance_divided_hundred_million: int = (
            account_ballance / atomic_to_coin_divider
        )
        account_ballance_divided_hundred_million_formated: str = self.format_coin_value(
...
def test_helper(
    ...
    electrumXTx_list: ElectrumXUnspentResponse = (
        print_account_balance_helper.balance_online_lookup(
