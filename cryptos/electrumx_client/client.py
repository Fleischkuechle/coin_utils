...
class NotificationSession(RPCSession):
    ...
    async def send_request_original(self, *args, timeout: int = None, **kwargs):
    ...
    async def send_request(
        ...
        **kwargs,
        ...
        """
        Asynchronously sends a request to the cryptocurrency network client,
        handling timeouts and errors.

        This method extends the base `send_request` method by adding timeout
        handling and error management.
        It utilizes asyncio's `wait_for` function to enforce a timeout for
        the request, and catches potential
        timeout errors (TaskTimeout, asyncio.TimeoutError) to raise a custom
        `RequestTimedOut` exception.

        Args:
            *args: Positional arguments to be passed to the underlying `send_request` method.
            timeout (int): The maximum time (in seconds) to wait for a response before timing out.
                        If not provided, the default timeout is used (likely defined elsewhere).
            **kwargs: Keyword arguments to be passed to the underlying `send_request` method.

        Returns:
            Any: The response object received from the network client, if the request completes within the timeout.

        Raises:
            RequestTimedOut: If the request times out before receiving a response.

        Note:
            - This method assumes that the underlying `send_request` method handles semaphores, backpressure,
            and other network-related mechanisms.
            - The `timeout` argument is typically not set directly here, as it is managed by the underlying
            network client (likely using aiorpcx).
            - The method uses a unique message ID (`msg_id`) to track the request and provide better error messages.
        """
        # note: semaphores/timeouts/backpressure etc are handled by
        # aiorpcx. the timeout arg here in most cases should not be set
        msg_id: int = next(self._msg_counter)
        try:
            # note: RPCSession.send_request raises TaskTimeout in case of a timeout.
            # TaskTimeout is a subclass of CancelledError, which is *suppressed* in TaskGroups
...
class ElectrumXClient:
    ...
    def __init__(
        ...
        servers: dict = read_json(f"servers/{server_file}", {})
        ...
        self._servers: Dict[str, Dict] = self.filter_servers_by_port(
            servers=servers,
            port_key=self._port_key,
        )
        self.servers: Dict[str, Dict] = self._servers
    ...
    def filter_servers_by_port(
        self,
        servers: Dict[str, Dict],
        port_key: str,
    ) -> Dict[str, Dict]:
        """
        Filters a dictionary of servers based on a specified port key.

        Args:
            servers (Dict[str, Dict]): A dictionary where keys are server names and values are dictionaries containing server information.
            port_key (str): The key in the server information dictionary that represents the port.

        Returns:
            Dict[str, Dict]: A new dictionary containing only the servers that have the specified port key.
        """

        filtered_servers = {}
        for k, v in servers.items():
            if v.get(port_key):
                filtered_servers[k] = v
    ...
    def _get_eligible_servers_original(self) -> Dict[str, Any]:
        return {
            k: v
    ...
    def _get_eligible_servers(self) -> Dict[str, Any]:
        """
        Returns a dictionary of eligible servers based on specified criteria.

        This function filters the servers in `self._servers` based on the following conditions:
        - The server must not be in the list of failed servers (`self._failed_servers`).
        - The server must have the specified port key (`self._port_key`) in its value dictionary.
        - The server address must end with ".onion" if `self._tor` is True, or it must not end with ".onion" if `self._tor` is False.

        For example, if `self._servers` is:
        ```python
        {
            "server1.example.com": {"port": 8080, "other_info": "some data"},
            "server2.onion": {"port": 9000, "other_info": "more data"},
            "server3.example.com": {"port": 80, "other_info": "even more data"},
        }
        ```
        and `self._failed_servers` is:
        ```python
        ["server1.example.com"]
        ```
        and `self._port_key` is "port" and `self._tor` is False, then the output of `_get_eligible_servers` would be:
        ```python
        {
            "server2.onion": {"port": 9000, "other_info": "more data"},
            "server3.example.com": {"port": 80, "other_info": "even more data"},
        ...
        Returns:
            Dict[str, Any]: A dictionary containing the eligible servers, where the keys are server addresses and the values are server information dictionaries.
        """
        eligible_servers: Dict[str, Any] = {}
        for k, v in self._servers.items():
            if (
                k not in self._failed_servers
                and self._port_key in v.keys()
                and (
                    self._tor
                    and k.endswith("onion")
    ...
    async def wait_new_start_original(self):
        async with self.restart_condition:
    ...
    async def _send_request_original(
    ...
    async def _send_request(
        ...
        **kwargs,
        ...
        This method acts as a wrapper for the underlying network client's request sending mechanism.
        It allows sending various types of requests (e.g., "blockchain.estimatefee") with optional arguments
        and a timeout value.

        Args:
            method (str): The method name of the request to be sent (e.g., "blockchain.estimatefee").
        ...
        Returns:
            Any: The response object received from the network client. The specific type of the response
                object depends on the method and the client implementation.

        Example:
            >>> response = await self._send_request("blockchain.estimatefee", numblocks=6)
            >>> print(response)
            {'fee_per_kb': 0.00009}  # Example response for Bitcoin

        Note:
            - This method assumes that the `session` attribute is an instance of a network client that
            supports asynchronous request sending.
            - The specific request format and response structure may vary depending on the network client
            implementation.
        """
        network_client_response_object: Any = await self.session.send_request(
            method, args, timeout=timeout, **kwargs
        )
        return network_client_response_object

    async def send_request_original(
        self,
        method: str,
        *args,
        timeout: int = 30,
    ...
    async def send_request(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            *args: Positional arguments to be passed to the request.
            timeout (int, optional): The timeout in seconds for the request. Defaults to 30.
            **kwargs: Keyword arguments to be passed to the request.

        Returns:
            Any: The response from the API endpoint.

        Raises:
            ConnectionError: If the connection to the API endpoint is not established.
        """
        await self._ensure_connected()
        network_client_response_object: Any = await self._send_request(
            method, *args, timeout=timeout, **kwargs
        )
        return network_client_response_object  # await self._send_request(method, *args, timeout=timeout, **kwargs)
    ...
    async def estimate_fee_original(self, numblocks: int = 6) -> float:
        return await self.send_request("blockchain.estimatefee", numblocks)

    async def estimate_fee(self, numblocks: int = 6) -> float:
        """
        Asynchronously estimates the transaction fee per kilobyte required for confirmation within
        a specified number of blocks for various cryptocurrencies.

        This function interacts with a cryptocurrency network client (e.g., an ElectrumX server)
        to obtain an estimated fee rate based on the current network conditions. The fee is
        expressed in the smallest unit of the cryptocurrency (e.g., satoshis for Bitcoin,
        dogecoins for Dogecoin, etc.) per kilobyte.

        Args:
            numblocks (int): The desired number of blocks within which the transaction should be
                            confirmed. The default value is 6, which represents a typical
                            confirmation time for many cryptocurrencies.

        Returns:
            float: The estimated fee per kilobyte in the smallest unit of the cryptocurrency.
                This value can be used to set the appropriate fee for a transaction to ensure
                timely confirmation.

        Example:
            >>> fee_per_kb = await self.estimate_fee(numblocks=3)
            >>> print(fee_per_kb)
            0.00009  # Example fee value in satoshis per kilobyte (for Bitcoin)
        ...
        """
        api_endpoint_response_object: Any = await self.send_request(
    ...
    async def unspent_original(self, scripthash: str) -> ElectrumXUnspentResponse:
    ...
    async def unspent(
        ...
        """
        Retrieves a list of Unspent Transaction Outputs (UTXOs) associated
        with the given script hash.
        by sending a online query self.send_request("blockchain.scripthash.listunspent", scripthash)

        Args:
            scripthash: The script hash for which to retrieve unspent outputs.

        Returns:
            An ElectrumXUnspentResponse (List[ElectrumXTx]) object containing information about the unspent outputs.
        """
        api_endpoint_response_object: Any = await self.send_request(
