...
def unspent(*addrs, coin_symbol=None, api_key=None, **kwargs):
    ...
    if len(addrs) == 1:
        txs = api.get_address_details(
            addrs[0], coin_symbol=coin_symbol, unspent_only=True, **kwargs
        )["txrefs"]
        for tx in txs:
            tx["output"] = "%s:%s" % (tx["tx_hash"], tx["tx_output_n"])
        return txs
    result = api.get_addresses_details(
        addrs, coin_symbol=coin_symbol, api_key=None, unspent_only=True, **kwargs
    )["txrefs"]
    return result
