...
class Create_Wallet_Watch_Only_Helper:
    def __init__(
        self,
        coin_symbol: str,
        testnet: bool = False,
    ...
    def make_wallet_watch_only_original(
        self,
        xpub_extended_public_key: str = "",
        print_result: bool = True,
    ) -> HDWallet:
        if xpub_extended_public_key == "":
        ...
        hd_wallet: HDWallet = coin.watch_wallet(xpub=xpub_extended_public_key)
        xpub = hd_wallet.keystore.xpub
        xprv = hd_wallet.keystore.xprv
        is_watching_only: bool = hd_wallet.is_watching_only
        new_receiving_address = hd_wallet.new_receiving_address()
        new_change_address = hd_wallet.new_change_address()
        print(f"pub_address: {xpub_extended_public_key}")
        ...
        # watch_wallet.preparetx(to=)
        if print_result:
            self.print_result(
                pub_address=xpub_extended_public_key,
            )
        # if coin.is_segwit_or_p2sh(addr=pub_address) == False:
        #     print(f"{self.coin_symbol} not a address: {pub_address}")
        coin_is_address: bool = coin.is_address(addr=xpub_extended_public_key)
        if coin_is_address == False:
            if self.coin_symbol == "doge":
                print(
                    f"TODO Check why it says that ->not a address: {xpub_extended_public_key}"
                )

        return hd_wallet

    def make_wallet_watch_only(
        self,
        xpub_extended_public_key: str = "",
        print_result: bool = True,
    ) -> HDWallet:
        """
        Creates a watch-only wallet using an extended public key (xpub).

        This function takes an extended public key (xpub) and creates a watch-only
        wallet for the specified cryptocurrency. Watch-only wallets allow you to view
        and interact with transactions on the blockchain without holding the private
        keys.

        Args:
            xpub_extended_public_key (str, optional): The extended public key (xpub)
                to use for the watch-only wallet. Defaults to "".
            print_result (bool, optional): Whether to print the results of the
                wallet creation process. Defaults to True.

        Returns:
            HDWallet: The newly created watch-only HDWallet object.

        Raises:
            None

        Example:
            >>> make_wallet_watch_only(
            ...     xpub_extended_public_key="xpub661MyMwAqRbcFtXgS5sYJABqqG9YLmC4Q1Rd5WfJ5uR5JaM1V"
            ...     "oY37N6g66m2YQGPwgGbHG8k64bP3t"
            ... )
            # Output:
            # xpub_address: xpub661MyMwAqRbcFtXgS5sYJABqqG9YLmC4Q1Rd5WfJ5uR5JaM1V"
            # "oY37N6g66m2YQGPwgGbHG8k64bP3t
            # hd_wallet.keystore.xpub: xpub661MyMwAqRbcFtXgS5sYJABqqG9YLmC4Q1Rd5WfJ5uR5JaM1V"
            # "oY37N6g66m2YQGPwgGbHG8k64bP3t
            # hd_wallet.keystore.xprv: None
            # hd_wallet.is_watching_only: True
            # hd_wallet.new_receiving_address(): ...
            # hd_wallet.new_change_address(): ...
        """
        if xpub_extended_public_key == "":
            print(f"xpub (extended public key) is empty...")
            return
        if not "xpub" in xpub_extended_public_key.lower():
            print("")
            print(self.line_symbol * self.line_length)
            print(
                f"xpub_extended_public_key is not a xpub format ({xpub_extended_public_key})"
            )
            print(
                f"make_wallet_watch_only expexts a xpub from an other already exisisting wallet that has an xpub "
            )
            print(
                f"you can get the xpub from an exisiting wallet with: (xpub:str = your_wallet.keystore.xpub)"
            )
            print(
                "Example xpub: xpub661MyMwAqRbcFtXgS5sYJABqqG9YLmC4Q1Rd5WfJ5uR5JaM1VoY37N6g66m2YQGPwgGbHG8k64bP3t"
            )
            print(self.line_symbol * self.line_length)
            return
        # ['btc', 'ltc', 'bch', 'dash', 'doge']
        ...
        is_watching_only: bool = hd_wallet.is_watching_only
        hd_wallet.keystore.can_import = True
        # new_receiving_address = hd_wallet.new_receiving_address()
        # new_change_address = hd_wallet.new_change_address()
        print(f"xpub_extended_public_key: {xpub_extended_public_key}")
        print(f"hd_wallet.keystore.xpub: {xpub}")
        print(f"hd_wallet.keystore.xprv: {xprv}")
        print(f"hd_wallet.is_watching_only: {is_watching_only}")
...
if __name__ == "__main__":
    coin_symbol: str = "btc"
    create_wallet_watch_only: Create_Wallet_Watch_Only_Helper = (
        Create_Wallet_Watch_Only_Helper(coin_symbol=coin_symbol)
    )
    # privkey: str = ""
    # pub_address: str = ""
    ...
    xpub_extended_public_key: str = (
        "xpub6C6Z9tiKL9QUSeqT94krvmjtsTQyEU7jEJWHEJdjF5Hy9Crtwj977DKKu5VwohUE3Zrnpv8fj3ZmjxcbDhMXd6jQRzhKm4Gdxxz7Wg2GrTZ"
    )
    watch_wallet: HDWallet = create_wallet_watch_only.make_wallet_watch_only(
        xpub_extended_public_key=xpub_extended_public_key
    )
    if watch_wallet:
        watch_wallet_addresses = watch_wallet.addresses
        # watch_wallet.preparetx()
        print(f"watch_wallet_addresses:{watch_wallet_addresses}")
