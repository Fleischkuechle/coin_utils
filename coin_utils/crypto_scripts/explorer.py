...
async def input_belongs_to_address(
    coin: BaseCoin, inp: Dict[str, Any], address: str, received: Dict[str, int]
) -> bool:
...
async def print_item(
    obj_id: str, coin_symbol: str = "btc", testnet: bool = False
) -> None:
    ...
    try:
        if tx_id := is_tx(coin, obj_id):
            ...
            history, unspent, balances = await asyncio.gather(
                coin.history(address), coin.unspent(address), coin.get_balance(address)
            )
            print("HISTORY:")
            if len(history) > 20:
                print("Last 20 transactions only")
            verbose_history = await asyncio.gather(
                *[coin.get_verbose_tx(h["tx_hash"]) for h in history[-21:-1]]
            )
            received = {}
            coinbase = False
            for h in verbose_history:
                ...
                for inp in h["vin"]:
                    ...
                    if not coinbase and await input_belongs_to_address(
                        coin, inp, address, received
                    ):
                        in_txid = inp["txid"]
                        outno = inp["vout"]
                        try:
            ...
            try:
                if coin_other_net.is_address(obj_id):
                    if testnet:
                        ...
                        message = (
                            f"{obj_id} is a testnet address. Try again with --testnet"
                        )
                else:
...
def main():
    ...
    parser.add_argument(
        "obj",
        help="Object to search for, either a transaction ID, block height or address",
    )
